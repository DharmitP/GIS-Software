...
#define ZOOM_FACTOR  1.05 /* For zooming on the graphics */
#endif
...
static void force_setlinewidth(double linewidth);
...
float xworld_to_scrn_fl(float worldx) {
...
float yworld_to_scrn_fl(float worldy) {
...
static void force_setlinewidth(double linewidth) {
    ...
    if (gl_state.disp_type == SCREEN) {
        ...
        fprintf(gl_state.ps, "%f setlinewidth\n", linewidth);
    }
}
...
void create_button(const char *prev_button_text,  char *button_text,
...
// Custom drawtext function for our searchbar
void drawtext_in_sb(const t_bound_box& bbox, const std::string& text) {
    t_point text_center = bbox.get_center();
    t_point bottomleft_bounds = text_center - bbox.bottom_left();
    t_point topright_bounds = bbox.top_right() - text_center;
    t_point min_bounds(
        min(bottomleft_bounds.x, topright_bounds.x),
        min(bottomleft_bounds.y, topright_bounds.y)
        );
    min_bounds *= 2;
    drawtext_l(bbox.get_center().x, bbox.get_center().y, text, min_bounds.x, min_bounds.y);
}
...
//Our team's custom drawtext function that aligns the base (for search bar)
void drawtext_l(float xc, float yc, const std::string& str_text, float boundx, float boundy) {
    // Need a C-string to call the low-level (X11 or win32) apis.
    const char* text = str_text.c_str();  
    int text_byte_length = strlen(text);
    float angle = PI * gl_state.currentfontrotation / 180.;
...
//    const int SEPARATOR_BUTTON_2 = 7;
//
//#ifdef X11
...
//        button_state.button[i].width = bwid;
//        button_state.button[i].enabled = 1;
//        if (i != SEPARATOR_BUTTON_INDEX && i != SEPARATOR_BUTTON_2) {
//            button_state.button[i].height = bheight;
//            y1 += bheight + space;
//        } else {
...
//    strcpy(button_state.button[7].text, "---");
...
//    button_state.button[7].fcn = NULL; // see 'adjustButton' below in WIN32 section
...
static void x11_drawbut(int bnum) {
    ...
    if (ispressed) {
        XSetForeground(x11_state.display, x11_state.gc_menus, 
            x11_convert_to_xcolor(t_color::predef_colors[ORANGE]));
    } else {
        XSetForeground(x11_state.display, x11_state.gc_menus, 
            x11_convert_to_xcolor(t_color::predef_colors[CYAN]));
        }
